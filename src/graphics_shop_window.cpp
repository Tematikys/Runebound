        ...
        if (create_buttons) {
            ...
            {  // SELL BUTTON
                ...
                Button button(
                    200, 30, HorizontalButtonTextureAlign::CENTER,
                    VerticalButtonTextureAlign::CENTER, 0, 0, texture,
                    [&is_selected = selected_plr_item,
                     &id = selected_plr_item_id, this]() {
                        if (is_selected != 0) {
    ...
    {  // UPDATE PRODUCTS
        ...
        for (auto id : m_network_client.get_game_client().m_shops.at(pos)) {
            ...
            {  // BORDER
                ...
                win->add_button(
                    "shop" + std::to_string(count + 1), button,
                    {5 + 305 * count, 5}, false, true
                );
            }  // BORDER
        ...
        for (auto id :
            ...
            {  // BORDER
                ...
                Button button(
                    300, height, HorizontalButtonTextureAlign::CENTER,
                    VerticalButtonTextureAlign::CENTER, 0, 0, temp,
                    [&num = selected_plr_item, &num_id = selected_plr_item_id,
                     id, count, this]() {
                        if (num == count + 1) {
                            num = 0;
                            num_id = 0;
                ...
                win->add_button(
                    "plr" + std::to_string(count + 1), button,
                    {5 + 305 * count, win->height() - 5 - height}, false, true
                );
            }  // BORDER
            SDL_SetRenderTarget(m_graphic_renderer, res);
            const SDL_Rect clip{0, 0, 300, height};
            ...
            Texture texture(res);
            win->add_texture(
                name, texture, {5 + 305 * count, win->height() - 5 - height},
                true
            );
            SDL_DestroyTexture(tex);
            ++count;
