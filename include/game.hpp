...
private:
  ...
  std::vector<std::shared_ptr<::runebound::character::Character>> m_characters;
...
public:
  ...
  std::vector<dice::HandDice>
  throw_movement_dice(const std::shared_ptr<character::Character> &chr) {
    check_turn(chr);
    m_last_dice_result =
  ...
  std::vector<dice::HandDice>
  throw_relax_dice(const std::shared_ptr<character::Character> &chr) {
    check_turn(chr);
  ...
  make_character(int gold, int health, const Point &current,
                 unsigned int hand_limit, unsigned int speed, std::string name,
                 const std::vector<::runebound::fight::FightToken> tokens) {
    m_characters.emplace_back(
        std::make_shared<::runebound::character::Character>(
            ::runebound::character::Character(gold, health, current, hand_limit,
                                              speed, std::move(name), tokens)));
    m_count_players += 1;
    return m_characters.back();
  }

  std::shared_ptr<::runebound::character::Character>
  make_character(const ::runebound::character::StandardCharacter &name);

  [[nodiscard]] int get_map_size() const { return m_map.get_size(); }

  [[nodiscard]] ::runebound::map::Map get_map() const { return m_map; }

  [[nodiscard]] Point get_position_character(
      const std::shared_ptr<character::Character> &chr) const;

  [[nodiscard]] unsigned int get_turn() const { return m_turn; }

  void check_and_get_card_adventure_because_of_token(
      std::shared_ptr<character::Character> chr);

  [[nodiscard]] std::vector<::runebound::dice::HandDice>
  throw_camping_dice(const std::shared_ptr<character::Character> &chr) const {
    return ::runebound::dice::get_combination_of_dice(chr->get_speed());
  }

  std::vector<Point>
  make_move(const std::shared_ptr<character::Character> &chr,
            const Point &point,
